...
class ProxyClient:
    ...
    def __init__(self, worker_url: Optional[str] = None):
        ...
        Args:
            worker_url: Cloudflare Worker URL. If None, reads from PROXY_WORKER_URL env var.
                       If that's also not set, direct requests are made (no proxy).
        """
        self.worker_url = worker_url or os.getenv("PROXY_WORKER_URL", "").strip()

...
def query_ai():
    ...
    # Use model from other_params or default
...
def fetch_all_bing_images(query, page=1):
    ...
    try:
        ...
        resp = requests.get("https://www.bing.com/images/search", headers=HEADERS, params=params, timeout=10)
...
def fetch_all_google_images(query, page=1):
    ...
    try:
        # Google uses 'ijn' parameter for pagination
...
def fetch_all_brave_images(query, page=1):
    ...
    try:
        ...
        resp = requests.get("https://search.brave.com/api/images", headers=HEADERS, params=params, timeout=10)
...
def index():
    ...
    has_next = total_results >= 10  # Assume there might be more results if we got 10 or more
    has_prev = page > 1
    ...
    for candidate in imgs[:10]:
        ...
        if img_path and os.path.exists(img_path):
            try:
                ...
                return send_file(os.path.abspath(img_path),
